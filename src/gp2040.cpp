...
GP2040::WebConfigHotkey::WebConfigHotkey()
    : active(false), noButtonsPressedTimeout(nil_time),
...
void GP2040::WebConfigHotkey::process(Gamepad *gamepad, bool configMode) {
  // We only allow the hotkey to trigger after we observed no buttons pressed
  // for a certain period of time. We do this to avoid detecting buttons that
  // are held during the boot process. In particular we want to avoid
  // oscillating between webconfig and default mode when the user keeps holding
  // the hotkey buttons.
  if (!active) {
    if (gamepad->state.buttons == 0) {
      if (is_nil_time(noButtonsPressedTimeout)) {
        noButtonsPressedTimeout =
            make_timeout_time_us(WEBCONFIG_HOTKEY_ACTIVATION_TIME_MS);
      }
    ...
    if (gamepad->state.buttons == webConfigHotkeyMask) {
      if (is_nil_time(webConfigHotkeyHoldTimeout)) {
        webConfigHotkeyHoldTimeout =
            make_timeout_time_ms(WEBCONFIG_HOTKEY_HOLD_TIME_MS);
      }

      if (time_reached(webConfigHotkeyHoldTimeout)) {
        // If we are in webconfig mode we go to gamepad mode and vice versa
        System::reboot(configMode ? System::BootMode::GAMEPAD
                                  : System::BootMode::WEBCONFIG);
      }
    } else {
      webConfigHotkeyHoldTimeout = nil_time;
    }
