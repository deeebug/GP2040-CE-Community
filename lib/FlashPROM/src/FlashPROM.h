...
#define OLD_EEPROM_SIZE_BYTES    4096           // Reserve 4k of flash memory (ensure this value is divisible by 256)
#define OLD_EEPROM_ADDRESS_START _u(0x101FF000)
...
{
	public:
		void start(bool migrate);
		void getOldVersionString(char* buffer, size_t offset, size_t length);
		void commit();
		void reset();
